...
class Schedule:
            ...
            if n <= len(MeetingTime.objects.all()):
                ...
                for course in courses:
                    for i in range(n // len(courses)):
                        ...
                        newClass = Class(self._classNumb, dept, section.id, course)
                        self._classNumb += 1
    ...
    def calculate_fitness(self):
        ...
        for i in range(len(classes)):
            if classes[i].room.room_capacity < int(classes[i].course.max_numb_students):
                self._numberOfConflicts += 1
            for j in range(len(classes)):
                if j >= i:
                    if (classes[i].meeting_time == classes[j].meeting_time) and \
                            (classes[i].id != classes[j].id) and (classes[i].section == classes[j].section):
...
def context_manager(schedule):
    ...
    cls = {}
    for i in range(len(classes)):
        cls["section"] = classes[i].id
        cls['dept'] = classes[i].department.dept_name
        cls['course'] = f'{classes[i].course.course_name} ({classes[i].course.id}, ' \
                        f'{classes[i].course.max_numb_students}'
        cls['room'] = f'{classes[i].room.room_number} ({classes[i].room.room_capacity})'
        cls['instructor'] = f'{classes[i].instructor.first_name} ({classes[i].instructor.id})'
        cls['meeting_time'] = [classes[i].meeting_time.id, classes[i].meeting_time.meeting_day, classes[i].meeting_time.meeting_time]
        context.append(cls)
    return context
